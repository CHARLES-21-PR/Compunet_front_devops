...
function CategoryView() {
    ...
    useEffect(() => {
        ...
        if (selectedBrands.length > 0) {
            result = result.filter(p => selectedBrands.includes(String(p.brand || '').trim()));
        }

        if (selectedColors.length > 0) {
            result = result.filter(p => selectedColors.includes(String(p.color || '').trim()));
        }
    ...
    const hasFilters = brands.length > 1 || colors.length > 1;
                    ...
                    <FormGroup>
                        {brands.map(brand => (
                            <FormControlLabel
                                ...
                                label={
                                    <Typography variant="body2" sx={{ cursor: 'pointer', width: '100%' }}>
                                        {brand}
                                    </Typography>
                                }
                            />
